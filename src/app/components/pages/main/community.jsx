...
const Community = () => {
   return (
      <div
         className="community w-full flex flex-col items-center mt-12 mb-20"
         id="community"
      >
         <Container>
            {/* Başlık */}
            ...
            <div className="grid lg:grid-cols-4 gap-8 grid-cols-2">
               {communityLinks.map((link, index) => (
                  <a
                     href={link.link}
                     className="flex-1 w-full bg-[#1f2a3f] p-6 rounded-lg flex flex-col items-center justify-center shadow-lg hover:-translate-y-2 transition-all"
                     key={index}
                  >
                     {link.icon}
